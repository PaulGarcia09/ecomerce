        ...
        return new Promise(function(resolve,reject){
            if(status !== undefined){
                ...
                con.connection.query(query1,function(error,results,fields){
                    let query = 'select p.id, tarjeta,p.correoelectronico ,vencimiento, cvv2, monto , codigosucursal, codigoprestamo, esvale, fecha, s.cardtype, s.cavv, s.status,s.xid, s.eci from informacionpp_pw2 p inner join informacion3dsecure s on p.id  = s.reference where p.id='+"'"+Referencia+"'";

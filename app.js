                    ...
                    libsodium.desencriptar(datainfo[0].ccv2).then(respuesta =>{
                     ...
                     pw2remates.ejecutarcobromultiple(vencimientodecrypt,ccvdecrypt,tarjetadecrypt,total,datainfo[0].upc,status,eci,xid,cavv).then(respuesta=>{
                        if(respuesta.resultado_payw == "A"){
                            ...
                            carrito.Obtenercarrito(orden).then(respuesta =>{
                                email.sendemailprueba(respuesta,cliente,celular,domicilio,colonia,cp,municipio,estado,instrucciones,CorreoPersonal,correoelectronico,orden)
...
app.get('/api/usuarios/obtenercodigorecupercion',upload.array(),(req,res)=>{
    let celular = req.body.celular ? req.body.celular: 0;
    let email = req.body.email ? req.body.email: "Novalido";

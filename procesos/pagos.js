...
let ejecutarprocesodepagos = async function consulta(){
    ...
    return new Promise(function(resolve,reject){
        ...
        request(url, function (error, response, body) {
            ...
            query ="SELECT r.id,reference,control_number,cust_req_date,auth_req_date,auth_rsp_date,cust_rsp_date,payw_result,auth_result,payw_code,auth_code,text,card_holder,issuing_bank,card_brand,card_type,r.tarjeta,r.correoelectronico,r.monto,r.codigosucursal,r.boleta,enviado,DATE_FORMAT(r.fecha,'%Y-%m-%d %T.%f') as fecha,p.diaspagados,p.codigotipopago FROM `respuestaspw2` r left join informacionpw2 p on p.id = r.control_number where r.id > 5550 and payw_result='A' and text='Aprobado' order by r.id asc"
